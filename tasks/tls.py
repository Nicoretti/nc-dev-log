...
@task
def ca(context, name='TestCA', destination='.'):
    """
    ...
    context.run(f"openssl req -x509 -new -nodes -key {keyfile} -sha256 -days 365 -out {certificate} -subj '{subject}'")


_SAN_CONFIG_TEMPLATE = cleandoc("""
    [req]
    default_bits  = 4096
...
def san_config(name, dns_entries=None, ip_entries=None):
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        with open(config, 'w') as f:
            f.write(_SAN_CONFIG_TEMPLATE.format(
                name=name,
                dns_entries="\n".join((f"DNS.{i} = {e}" for i, e in enumerate(dns_entries, 1))),
                ip_entries="\n".join((f"IP.{i} = {e}" for i, e in enumerate(ip_entries, 1))),
            ))
        yield config


@task(iterable=["dns", "ip"])
def server(context, name='Test Server', destination='.', dns=None, ip=None, root_cert=_ROOT_CERT, root_key=_ROOT_KEY):
    """
